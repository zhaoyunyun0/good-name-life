...
- 生产字库：离线收录8105个规范汉字，其中903个通过姓名适用校验并进入推荐池；完整四柱、气质和重名风险共同参与筛选。
...
项目默认读取仓库内的离线8105字数据，不需要运行时联网。未配置其他授权外部数据时，程序使用离线字库和透明风险模型，不伪造在线结果。
...
- 基础字库未收录或缺少五行数据的汉字会明确报错，不使用随机笔画或五行兜底。
