...
def save_history(feature: str, input_text: str, result: dict) -> None:
    with sqlite3.connect(DB_PATH) as db:
...
def char_profile(char: str) -> tuple[int, str, str]:
    ...
    if char in COMMON_SURNAME_STROKES:
        ...
        return strokes, element, "按姓名学笔画数理归类"
...
def evaluate_name(name: str, birth: date, birth_time: tuple[int, int], gender: str,
    ...
    if bazi["missing"]:
        covered = len(set(bazi["missing"]) & set(given_elements))
        element_score = round(62 + 36 * covered / len(bazi["missing"]))
    else:
    ...
    total = round(pronunciation * .18 + shape * .14 + meaning * .18 + element_score * .35 + trigram_score * .15)
...
def score_name(data: dict) -> dict:
    ...
    result = {
        ...
        "calculation": {"tones": assessment["tones"], "strokes": assessment["strokes"],
                        "weights": "音韵18% + 字形14% + 寓意18% + 五行35% + 卦象15%"},
    ...
    save_history("score", name, result)
    return result
...
def generate_names(data: dict) -> dict:
    ...
    for first in chars:
        for second in chars:
            ...
            raw_candidates.append((first, second, elements, style_matches, fixed_match))

    # 风格只用于缩小候选范围，不进入评分；限制精评数量以避免重复拼音计算。
    rng.shuffle(raw_candidates)
    raw_candidates.sort(key=lambda item: (item[4], item[3]), reverse=True)
    candidates = []
    for first, second, elements, style_matches, fixed_match in raw_candidates[:120]:
    ...
    candidates.sort(key=lambda item: (item["_fixed_rank"], item["_style_rank"], item["score"]), reverse=True)
    ...
    if bazi["missing"]:
        ...
        analysis = (f"八个干支本气中五行齐全，没有计数为0的元素；{favorable_text}数量相对最少。"
                    f"本次优先选用{favorable_text}属性字，并参考相生的{support_text}属性字保持组合平衡。")
    if fixed_char:
        analysis += f" 已将“{fixed_char}”设为优先用字；在不破坏补五行和质量要求的前提下尽量采用。"
    strategy = {"analysis": analysis, "required": bazi["missing"][:2],
    ...
    save_history("naming", surname, result)
    return result
...
def population(data: dict) -> dict:
    ...
    result = {"name": name, "score": score, "level": level, "label": label,
              "confidence": confidence, "signals": signals,
              "method": "姓氏分档 + 高频用字 + 名字长度 + 叠字 + 常见完整姓名",
              "disclaimer": "这是组合稀有度风险评估，不是公安人口查询，不提供虚构的同名人数。"}
    save_history("duplicate_risk", name, result)
    return result
