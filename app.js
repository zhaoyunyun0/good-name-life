...
document.querySelector('#naming-form').addEventListener('submit',async e=>{
  ...
  try{
    ...
    target.innerHTML=`<div class="naming-analysis"><p class="eyebrow">生辰八字与五行分析</p><h3>${d.bazi.pillars.map(esc).join('　')}</h3><p class="lunar-date">${esc(d.bazi.lunar_date)} · 日主属${d.bazi.day_master} · 月令属${d.bazi.seasonal}</p><div class="element-counts">${counts}</div><div class="analysis-copy"><b>分析结论：五行缺失 ${missing}</b><p>${esc(d.strategy.analysis)}</p><small>起名方向：优先 ${d.strategy.recommended.join('、')} · 相生辅助 ${d.strategy.supporting.join('、')||'无'}</small><small>离线基础字库 ${d.strategy.base_character_count} 字 · 起名精选池 ${d.strategy.eligible_character_count} 字</small><small>本次参与 ${d.strategy.character_pool_size} 字 · ${d.strategy.valid_combinations} 组有效组合 · 精评 ${d.strategy.evaluated_candidates} 组</small><small>${esc(d.strategy.diversity_note)}</small><small class="source-line">姓名语料：${esc(d.strategy.corpus_source)}</small></div></div><p class="eyebrow name-list-title">结合以上分析推荐十组名字</p><div class="name-grid">${d.names.map(n=>`<div class="name-item"><b>${esc(n.name)}</b><span>${n.score}</span><p>${esc(n.meaning)}</p><small>用字五行：${n.elements.join('、')} · ${esc(n.trigram)}</small><small>期望气质命中 ${n.style_matches} 字 · 重名风险：${esc(n.duplicate_risk.label)}（${n.duplicate_risk.score}）</small>${n.ambiguity_warnings.length?`<small class="warning-line">歧义提示：${n.ambiguity_warnings.map(esc).join('、')}</small>`:''}<div class="name-actions"><button class="name-score-link" data-action="score-name" data-score-name="${esc(n.name)}">查看评分 →</button><button class="name-score-link" data-action="favorite-name" data-score-name="${esc(n.name)}">＋ 收藏</button><button class="name-score-link" data-action="to-ai-candidate" data-score-name="${esc(n.name)}">AI 顾问 →</button></div></div>`).join('')}</div>`;
  }catch(_){}
});
...
document.addEventListener('click',e=>{
  ...
  if(action.dataset.action==='favorite-name'&&lastNamingInput){
    const item=generatedCandidates.get(action.dataset.scoreName);if(!item)return;const items=getFavorites(),id=[item.name,lastNamingInput.birth,lastNamingInput.birth_time,lastNamingInput.style].join('|');if(!items.some(x=>x.id===id))items.push({...item,id,input:lastNamingInput});setFavorites(items);action.textContent='✓ 已收藏';
  }
  ...
  if(action.dataset.action==='to-ai-score'&&lastScoreInput){const form=document.querySelector('#ai-form');form.elements.name.value=lastScoreInput.name;form.elements.birth.value=lastScoreInput.birth;form.elements.birth_time.value=lastScoreInput.birth_time;form.elements.gender.value=lastScoreInput.gender;showPage('ai');if(aiStatus.enabled)setTimeout(()=>form.requestSubmit(),80)}
  if(action.dataset.action==='to-ai-candidate'&&lastNamingInput){const form=document.querySelector('#ai-form');form.elements.name.value=action.dataset.scoreName;form.elements.birth.value=lastNamingInput.birth;form.elements.birth_time.value=lastNamingInput.birth_time;form.elements.gender.value=toScoreGender(lastNamingInput.gender);showPage('ai');if(aiStatus.enabled)setTimeout(()=>form.requestSubmit(),80)}
});
